      ...
      <CardContent>
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="timeline">
            <div className="py-4">
              <TimelineView span={span} transaction={transaction} />
            </div>
          </TabsContent>
        </Tabs>
      </CardContent>
...
function TimelineView({
  ...
  const offsetPercentage = ((spanStart - transactionStart) / safeTotal) * 100;
  ...
  console.log({
    ...
    spanStart,
    spanEnd,
    offsetPercentage: displayOffsetPercentage,
    widthPercentage: displayWidthPercentage,
    spanDuration: span.durationMs,
  });

  return (
    <div className="space-y-4">
      <div className="relative h-8 bg-muted/30 rounded-md w-full">
        <div
          className="absolute top-0 h-full bg-blue-500/20 rounded-md"
          style={{
      ...
      <div className="flex justify-between text-xs text-muted-foreground">
        <div>트랜잭션 시작</div>
        <div>
          {formatDuration(totalDuration <= 0 ? span.durationMs : totalDuration)}
        </div>
        <div>트랜잭션 종료</div>
      </div>
      ...
      <div className="space-y-2 pt-4">
        <InfoItem
          label="span 시작"
          value={`트랜잭션 시작 후 ${formatDuration(spanStart - transactionStart)} (${displayOffsetPercentage.toFixed(1)}%)`}
        />
        <InfoItem
          label="span 종료"
          value={`트랜잭션 시작 후 ${formatDuration(spanEnd - transactionStart)} (${(displayOffsetPercentage + displayWidthPercentage).toFixed(1)}%)`}
        />
        <InfoItem
          label="span 지속 시간"
          value={`${formatDuration(span.durationMs)} (전체 지속 시간의 ${displayWidthPercentage.toFixed(1)}%)`}
        />
      </div>
    </div>
  );
